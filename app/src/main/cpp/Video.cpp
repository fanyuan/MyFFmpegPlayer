//
// Created by develop on 2021/2/11.
//
#include <jni.h>
#include <string>
...
Java_com_example_myffmpeg_FfmpegVideoUtil_getTestString(JNIEnv *env, jclass clazz) {
    return env->NewStringUTF("ffmpeg String test");
}
extern "C"
JNIEXPORT void JNICALL
Java_com_example_myffmpeg_MyPlayer_native_1start(JNIEnv *env, jobject thiz, jstring path_,
                                                 jobject surface) {
    ...
    while (av_read_frame(formatContext,packet)>=0){
        ...
        if(packet -> stream_index == video_stream_idx){
            ...
            if(ret == 0){
                ...
                //拿到一行有多少个字节RGBA
                int destStide = outBuffer.stride*4;
                uint8_t *src_data = dst_data[0];
                int src_linesize = dst_linesize[0];
                uint8_t *firstWindown = static_cast<uint8_t *>(outBuffer.bits);
                for(int i = 0; i < outBuffer.height;++i){
                    memcpy(firstWindown + i * destStide,src_data + i * src_linesize,destStide);
                }

//
// Created by develop on 2021/2/12.
//
#include <jni.h>
#include <string>
...
Java_com_example_myffmpeg_AudioDecodeUtil_decode(JNIEnv *env, jobject thiz, jstring jinput,
jstring joutput) {
    ...
    if(avformat_open_input(&formatContext,input,NULL,NULL)!=0){
        LOGI("%s %s","无法打开音频文件",input)
        return;
    }
    //获取输入文件信息
    if(avformat_find_stream_info(formatContext,NULL) < 0){
        LOGI("%s %s","无法获取输入文件信息",input)
        return;
    }
